...
option_list = list(
  ...
  make_option("--chrom", action="store", default="1-22", type='character',
              help="The chromosome on which the model is fitted, one chrom only[required]"),
...
results <- data.frame(chr = integer(), iter = integer(), eta = numeric(), alpha = numeric(), R2 = numeric())
...
alphas <- 2^-seq(1,14,by =2)  # Use same alpha for all
...
for (iter in iters) {
  ...
  for (eta in etas) {
    for (alpha in alphas) {
      ...
      fwrite2(score, paste0(opt$PATH_out, "/before_ensemble/chr", allchrom, "_score_file.txt"), 
      ...
      if(opt$testing){
        ...
        arg <- paste0(opt$PATH_plink, " --threads ", NCORES,
                      ...
                      " --score ", opt$PATH_out, "/before_ensemble/chr", allchrom, "_score_file.txt header-read",
                      " cols=+scoresums,-scoreavgs --score-col-nums 4",
                      " --out ", opt$PATH_out, "/tmp/sample_scores_", ethnic[1], "/chr", allchrom, "_before_ensemble_testing")
        ...
        # Check for output file with chromosome number
        if (!file.exists(paste0(opt$PATH_out, "/tmp/sample_scores_", ethnic[1], "/chr", allchrom, "_before_ensemble_testing.sscore"))) {
          stop("Error: PLINK output file does not exist. Check if PLINK ran successfully.")
        }
        
        # Read the output using fread2
        SCORE <- fread2(paste0(opt$PATH_out, "/tmp/sample_scores_", ethnic[1], "/chr", allchrom, "_before_ensemble_testing.sscore"))
