...
option_list = list(
  ...
  make_option("--Lcr", action="store", default=5, type='integer',
              help="Length of path for the additional tuning parameter c for AFR [default: %default]"),
...
Ll <- opt$Ll
Lc <- opt$Lc
Lcr <- ifelse("AFR" %in% ethnic,opt$Lcr,1)
Ngridsearch <- Ll*Lc*Lcr
...
tmp <- exp(seq(from=0,to=log(3),length.out=ceiling((Lcr+1)/2)))
cr_path <- c(rev(1/tmp),tmp[-1])
...
for (lc in 1:Lc){
  ...
  m <- which(ethnic=="AFR")
  for(i in 1:Lcr){
    tmp[m,] <- cr_path[i] * cpath_tmp[lc]
    tmp[,m] <- cr_path[i] * cpath_tmp[lc]
    cpath[[(lc-1)*length(cr_path)+i]] <- tmp
  }
}
